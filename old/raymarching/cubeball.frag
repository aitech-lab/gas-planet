...
void main( void )  {
	...
	if (abs(p.y) < 0.7) {
    	...
    	if (dist < 100.0) {
    		...
    		float diff1 = 1.0*clamp(dot(n, normalize(vec3(1.0,1.0,1.0))), 0.0, 1.0); 
    		...
    		float amb = 0.2 + pos.y; 
    		color = diff1*vec3(1,1,1) + diff2*vec3(1,0,0)  + diff3*vec3(1,0,1) + spec*vec3(1,1,1)  + amb*vec3(0.2,0.2,0.2); 
    		color /= dist;
    	}
	}
